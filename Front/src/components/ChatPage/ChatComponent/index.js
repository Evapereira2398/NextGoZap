...
import { ChatLayout, DocumentComponent, ImageContainer, MessageContainer, MessageContent, MessageContentText, StickerComponent} from "./style";
...
const ChatComponent = ({ message, session, isMe, isWarning, selectMessageId}) => {
  ...
  function getBodyMessage() {
    ...
    if (!!message?.body && message.type === "gp2" && message.subtype === "picture") {
  ...
  return (
    <ChatLayout side={isMe}>
         <ImageModal open={openModalImage} handleClose={handleCloseModalImage} message={message} image={clickedUrl}/>

         <MessageContainer side={isMe} isWarning={isWarning}>
            
            <div style={{ position: "absolute", right: -15, top: -15, zIndex: 10 }}>
            ...
            <MessageContent side={isMe} isWarning={isWarning}>
               ...
               {message.isMedia ? (message.type === "video" ? (

                  <ImageContainer>
                     <video ref={imageRef} src={`data:image/png;base64, ${message.body}`} controls />
                     <div className={"download"} style={{ display: display }} onClick={() => onClickDownload("video")} />
                  </ImageContainer>
                     ...
                     <ImageContainer>
                        <img
                           ref={imageRef}
                           src={`data:${message.mimetype};base64, ${message.body}`}
                           loading={"lazy"}
                           alt={message.caption}
                           onClick={handleOpenModalImage}
                           onError={(e) => {
                              e.target.onerror = null; 
                              e.target.src = defaultImage; // Define uma imagem padrão em caso de erro
                           }}
                        />
