...
const SendMessagePage = () => {
    ...
    try {
      ...
      for (const contact of arr) {
        const [imageResponse, statusResponse] = await Promise.all([
          ...
          api.get(
            `${getSession()}/profile-status/${contact.id.user}`,
            config()
          ),
        ]);
        ...
        if (
          ...
          imageResponse.data.response.eurl
        ) {
  ...
  async function getAllChats() {
    try {
        ...
        const contactsObject = contactsResponse.reduce((acc, contact) => { acc[contact.id._serialized] = contact;
            return acc;
        }, {});
        ...
        const arr = chatsResponse.map((chat) => ({
            ...
            const {
                data: { response }} = await api.get(`${getSession()}/all-chats-with-messages`, config());
  ...
  async function getChatImages() {
    try {
      ...
      return data.response.reduce((acc, chat) => {
        acc[chat.id._serialized] =
          chat.contact.profilePicThumbObj?.img || "./user.png";
        return acc;
      }, {});
    } catch (error) {
  ...
  const onClickContact = useCallback(async (contact) => {
        ...
        const phone = contact.id.replace(isGroup ? "@g.us" : "@c.us", "");

        try {
            const chatData = await api.get(`${getSession()}/chat-by-id/${phone}?isGroup=${isGroup}`, config());
            await api.post(`${getSession()}/send-seen`, { phone }, config());
                  ...
                  <div className={"container-info-ctt"}>
                    ...
                    <h3>
                      {!choosedContact.name === undefined
                        ? choosedContact.id.replace(/[@c.us,@g.us]/g, "")
                        : choosedContact.name}
                    ...
                    <div className="content">
                      <ChatComponent
                        ...
                        isWarning={
                          ...
                          !["ptt", "audio"].includes(selectedMessage.type)
                        }
                        session={getSession()}
                        token={getToken()}
